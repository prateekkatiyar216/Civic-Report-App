...
export default function ReportIssueScreen({ navigation }: any) {
  ...
  const handleSubmit = async () => {
    if (!title.trim()) { Alert.alert("Missing field", "Please add a title."); return; }
    if (!description.trim()) { Alert.alert("Missing field", "Please add a description."); return; }
    if (!category) { Alert.alert("Missing field", "Please select a category."); return; }
    if (!image) { Alert.alert("Missing field", "Please add a photo."); return; }
    if (!location) { Alert.alert("Location unavailable", "Waiting for GPS location..."); return; }
...
const styles = StyleSheet.create({
  ...
  imageUploader: {
    ...
    borderStyle: "dashed", backgroundColor: "#13131e",
  },
  imagePlaceholder: {
    height: 180, justifyContent: "center", alignItems: "center", gap: 6,
  },
  ...
  categoryItemActive: {
    backgroundColor: "#1a2a10", borderColor: "#c8f04a",
  },
  ...
  locationBadge: {
    marginLeft: "auto", backgroundColor: "#1a2a10",
    borderRadius: 8, paddingVertical: 4, paddingHorizontal: 8,
    borderWidth: 1, borderColor: "#2a4a1a",
  },
